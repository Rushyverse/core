...
(
    ...
    PRIMARY KEY (id),
    FOREIGN KEY (owner_id) REFERENCES player (uuid) ON DELETE CASCADE ON UPDATE CASCADE
);
...
(
    ...
    FOREIGN KEY (entity_id) REFERENCES player (uuid) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE
OR REPLACE VIEW guild_members_with_owner AS
...
CREATE
OR REPLACE FUNCTION check_member_is_owner() RETURNS TRIGGER AS
$$
BEGIN
    IF
EXISTS(SELECT 1
...
CREATE
OR REPLACE TRIGGER check_member_is_owner_trigger
    BEFORE INSERT
    ON guild_member
    FOR EACH ROW
...
CREATE
OR REPLACE FUNCTION delete_invite() RETURNS TRIGGER AS
$$
BEGIN
DELETE
FROM guild_invite
...
CREATE
OR REPLACE TRIGGER delete_invite_trigger
    AFTER INSERT
    ON guild_member
    FOR EACH ROW
...
CREATE
OR REPLACE FUNCTION is_member(guild INTEGER, entity uuid)
    RETURNS TABLE
            (
                is_member INTEGER
            )
...
CREATE
OR REPLACE FUNCTION check_existing_member() RETURNS TRIGGER AS
$$
BEGIN
    IF
EXISTS(SELECT 1 FROM is_member(NEW.guild_id, NEW.entity_id)) THEN
        RAISE EXCEPTION USING
            ERRCODE = 'P1000',
...
CREATE
OR REPLACE TRIGGER check_existing_member_trigger
    BEFORE INSERT
    ON guild_invite
    FOR EACH ROW
...
CREATE
OR REPLACE FUNCTION delete_expired_invite() RETURNS TRIGGER AS
$$
BEGIN
DELETE
FROM guild_invite
...
CREATE
OR REPLACE TRIGGER delete_expired_invite_trigger
    AFTER INSERT OR
UPDATE
    ON guild_invite
    FOR EACH STATEMENT
